...
const displayToggle = (id,dtype) => {
    ...
    let newClass, c = document.getElementById(dtype + '-' + id).className;
...
const TheSearchResult = props => {
    let titleFormated = props.title.charAt(0).toUpperCase() + props.title.slice(1)
    let thecode = props.code.toString()
        .replace(/&lt;/g,'<')
        .replace(/&gt;/g,'>')
    return (
        <div className={"rowdata " + props.bgc} >
            <Grid container spacing={1}  >
                <Grid item xs={12} sm={10} >
                    ...
                    <div justify="flex-end" className="displayNone" id={"etitle-" + props.id} style={lgBg}>
                        <TextField
                        ...
                        <div style={searchTitle}  className="displayBlock" id={"title-" + props.id} onClick={() => { 
                ...
                <Grid item xs={3} sm={2} >
                    <div justify="flex-end" className="displayNone" id={"cmd-" + props.id}>
                        <ButtonGroup variant="contained" color="primary" aria-label="contained primary button group">
                            <Button onClick={ () => props.editEntryStart(props.id)} >Edit</Button>
                            <Button onClick={ () => props.removeEntryStart(props.id)}>Delete</Button>
                        </ButtonGroup>
                    </div>
                </Grid>
                <Grid item xs={12} >
                    <div className="displayNone" id={"t-" + props.id}>
                        <div id={'intro-' +props.id} className="displayBlock">
                            <pre>{props.intro}</pre><br />
                        </div>
                        <div id={'eintro-' +props.id} className="displayNone" style={lgBg}>
                            <TextField
                        ...
                        <pre>
                            <code className="html" className="displayBlock" id={"code-" + props.id}>
                                {thecode}
                            </code>
                        </pre>

                        <div id={'ecode-' +props.id} className="displayNone" style={lgBg}>
                        ...
                        <div style={{padding:5, justify:'center'}} className="displayNone" id={'save-' + props.id} >
                            <ButtonGroup variant="contained" color="secondary" aria-label="contained primary button group">
                                <Button onClick={ () => props.updEntryStart(props.id)} >Save Changes</Button>
                                <Button onClick={ () => props.editEntryStart(props.id)}>Cancel</Button>
                            </ButtonGroup>
                        </div>                        
                    </div>
                </Grid>
...
const AllSearchRes = props => {
    ...
    if(props.searchResults !== undefined && Array.isArray(props.searchResults)){
    ...
    return (
      <div>
        { 
            props.searchResults.map(result => <TheSearchResult
                id={result.id}
                key={result.id}
                title={result.title}
                ttype={result.ttype}
                ...
                editEntryStart={props.editEntryStart}
